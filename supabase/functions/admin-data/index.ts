  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (body.action === "approve_verification" || body.action === "reject_verification") {
        ...
        const updateData: any = { status, reviewed_by: user.id, reviewed_at: new Date().toISOString() };
      ...
      if (body.action === "confirm_payment" || body.action === "reject_payment") {
        const payment_status = body.action === "confirm_payment" ? "confirmed" : "rejected";
        const updateData: any = { payment_status };
        // If confirming payment, also confirm the order
